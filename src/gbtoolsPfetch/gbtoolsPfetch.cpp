...
static void pfetch_handle_class_init(PFetchHandleClass pfetch_class)
{
  ...
  handle_class->handle_signals[HANDLE_READER_SIGNAL] =
    g_signal_new("reader", 	/* signal_name */
		 ...
		 gbtoolsPfetch_cmarshal_ENUM__STRING_POINTER_POINTER, /* marshal */
  ...
  handle_class->handle_signals[HANDLE_CLOSED_SIGNAL] =
    g_signal_new("closed", 
		 ...
		 NULL, 		/* accumulator */
		 NULL, 		/* accumulator data */
		 gbtoolsPfetch_cmarshal_ENUM__VOID, /* marshal */
		 PFETCH_TYPE_HANDLE_STATUS,	/* return type */
		 0,	/* number of params */
		 G_TYPE_STRING,
		 G_TYPE_POINTER,
	  ...
	  if(curl_object_error)
            {
              ...
              g_warning("%s\n", curl_object_error);
              g_set_error(error, g_quark_from_string("libpfetch"), 1, "%s\n", curl_object_error) ;
